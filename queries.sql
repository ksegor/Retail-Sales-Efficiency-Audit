...
3. Группировка данных:
* GROUP BY e.employee_id, s.sale_date: Группирует результаты по идентификатору сотрудника и дню недели, чтобы агрегировать данные по каждому продавцу и каждому дню.

4. Сортировка результатов:
* ORDER BY EXTRACT(DOW FROM s.sale_date), seller: Сортирует результаты по порядковому номеру дня недели (где 0 — воскресенье, 1 — понедельник и т.д.) и по имени продавца.
